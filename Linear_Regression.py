...
class StudentAgent:
    def __init__(self, max_depth=4):
        ...
        # Enhance important pattern weights
        for i in range(8):  # 8 win patterns
            ...
            if p1_two_idx < len(self.weights):
                self.weights[p1_two_idx] *= 1.8  # Increased from 1.5 to 1.8
            if p2_two_idx < len(self.weights):
                self.weights[p2_two_idx] *= 1.8  # Increased from 1.5 to 1.8
        
        # Board control (indexes 36-38)
        center_board_idx = 36  # Center board control
        if center_board_idx < len(self.weights):
            self.weights[center_board_idx] *= 2.5  # Increased from 2.0 to 2.5
        
        # Direct board count differential (index 35)
        # UPDATED: Instead of multiplying the negative value by 1.3, we replace with a positive value
        board_count_idx = 35  # Board count differential
        if board_count_idx < len(self.weights):
            self.weights[board_count_idx] = 0.25  # Set to positive value instead of multiplying negative value
